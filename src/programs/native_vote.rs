...
pub async fn fragment_instruction(
    ...
    let vdr = limited_deserialize(
        instruction.data.as_slice());

    // Always deserialize the instruction first, since we should always trust the blockchain
    if let Ok(deserialized_instruction) = vdr {
        // Find out what instruction this is
        return match deserialized_instruction {
        ...
        warn!("Invalid instruction for the vote program.");
        None
    }
}